...
Each finding carries the file, the line, what is wrong in one sentence, the fix, and **whether the fix belongs in this branch or in an issue of its own** — that is the choice `/ship` has to declare for every finding.
