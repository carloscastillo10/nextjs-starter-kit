...
Three mechanisms, chosen per item:
...
- **Skills written here** hold what only this repository can say. They are edited by hand, like any other document, and reviewed with the code.
...
| File                                                                                  | Role                                                                                                        |
| ------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- |
| `<skill>/`                                                                            | One vendored skill each, except `project-conventions/`. Do not edit a vendored one by hand.                 |
...
Every skill in this folder loads on its own. Claude picks one when its description matches the task, and you can call it by name, for example `/design-md`.
...
**Write a skill here instead** when the guidance is about this repository and nothing published can carry it: the conventions, the shape of a workflow, a decision that only holds here. Keep it short and point at the document that owns the rule, so the two cannot drift; a skill long enough to argue with itself is a document in the wrong folder.

Also:

- Do not edit a vendored skill folder by hand. Refresh through the CLI so the hash in `skills-lock.json` stays valid. A skill written here is edited like any other document.
...
| `design-taste-frontend`, `high-end-visual-design`                                                  | Discourage or ban the shadcn/ui defaults: `lucide-react`, Inter, `shadow-md`. The first is worth adding to a project with a marketing surface; see [Optional skills](#optional-skills).                                                                                                                                          |
...
- [Claude Code hooks](../../tooling/scripts/claude-hooks/README.md): the skill reminder and how to declare a hook.
