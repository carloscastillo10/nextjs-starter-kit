...
- Three shapes do not extend a root: a closed component, a provider in `_app/providers`, and what the router renders. A route file's component and the page component it re-exports take `LayoutProps` / `PageProps` and spread nothing, because what Next.js hands them is promises and the DOM would receive them.
- `className` first and callbacks last, in the props type, in the destructuring, in the JSX and in what a hook returns, and alphabetical inside each group everywhere but the JSX. The linter fixes all four.
- A hook returns a plain object, never one wrapped in `useMemo`, and `useMemo` takes no type argument: the factory already fixes the type. No `React.memo` without a measured reason. A `style` object is memoized in the hook, or is a module constant when it never changes.
